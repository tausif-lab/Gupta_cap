...
const createPaymentRequest = async (req, res) => {
  try {
    const { userId, userName, mobile, flat, monthlyRent, penaltyAmount, totalPaid, dueDate, cycleStart, cycleMonthLabel } = req.body;

if (!userId || !totalPaid || !dueDate || !cycleStart || !cycleMonthLabel) {
...
const request = await PaymentRequest.create({
  ...
  cycleMonthLabel,
    ...
    res.status(201).json({ message: 'Payment request submitted for verification', request });
  } catch (error) {
    console.error('CREATE PAYMENT REQUEST ERROR:', error);
    res.status(500).json({ message: 'Failed to submit payment request', error: error.message });
  }
};
...
const getPaymentStatus = async (req, res) => {
  try {
    const latestRequest = await PaymentRequest.findOne({ userId: req.params.userId })
      .sort({ createdAt: -1 });
...
const getTenantPaymentRequests = async (req, res) => {
  try {
    const requests = await PaymentRequest.find({ userId: req.params.userId })
      .sort({ createdAt: -1 });
    res.json({ requests });
  } catch (error) {
...
module.exports = { createPaymentRequest,verifyPaymentRequest , getPaymentStatus , getTenantPaymentRequests };