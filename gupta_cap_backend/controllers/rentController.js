...
const getRentConfig = async (req, res) => {
  try {
    ...
    if (!config) {
      return res.status(404).json({ message: 'No rent config found for this tenant' });
    }
    res.json(config);
  } catch (error) {
    console.error('GET RENT CONFIG ERROR:', error);
    res.status(500).json({ message: 'Failed to fetch rent config', error: error.message });
  }
};
...
const saveRentConfig = async (req, res) => {
  try {
    const { rentStartDate, dueDays, penaltyStartDay, monthlyRent, penaltyPerDay, penaltyEnabled } = req.body;

    if (!rentStartDate || !monthlyRent) {
      return res.status(400).json({ message: 'Rent start date and monthly rent are required' });
    }
    ...
    res.status(500).json({ message: 'Failed to save rent config', error: error.message });
  }
};
